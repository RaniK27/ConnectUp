...
router.get('/signup_student',async(req,res)=>{
    res.locals.session = req.session;
    const message = req.flash('status');
    res.render('signup_student',{ message })
});
...
router.post('/submit_student',upload.single('image'),async(req,res)=>{  
    const salt = 5;
    ...
    mysqlConn.query(sql,(err,rows,fields)=>{
        ...
        var sql2 = `INSERT INTO student_1 VALUES("${req.body.name}","${req.body.qualification}","${req.body.domain}","${req.body.intdomain}","${req.body.institute}","${req.body.ln}","${req.body.project}")`
